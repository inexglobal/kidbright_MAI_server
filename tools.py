...
def iou_score(bboxes_a, bboxes_b):
    ...
    s = area_a + area_b - area_i
    s[s==0] = 1e-12
    cross = area_i
    # a = torch.sum(torch.isinf(s))
    # b = torch.sum(torch.isinf(cross))
    # if a > 0 or b > 0:
    #     print(s, cross)
    #     print("---", a, b)
    return  cross / s
...
def loss(pred_conf, pred_cls, pred_txtytwth, label, num_classes, obj_loss_f='mse'):
    ...
    if torch.isnan(conf_loss):
        ...
        print(gt_conf)
        print(gt_obj)
        print(torch.sum(torch.isnan(pred_conf)), torch.sum(torch.isnan(gt_conf)), torch.sum(torch.isnan(gt_obj)))
        print("---")
